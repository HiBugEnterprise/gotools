...
type OrderNotifyCheckSignRequest struct {
	PayOrderId     string `json:"payOrderId,omitempty"`     // 支付订单号
	MchNo          string `json:"mchNo,omitempty"`          // 商户号
	AppId          string `json:"appId,omitempty"`          // 应用ID
	MchOrderNo     string `json:"mchOrderNo,omitempty"`     // 商户订单号
	IfCode         string `json:"ifCode,omitempty"`         // 支付接口编码
	WayCode        string `json:"wayCode,omitempty"`        // 支付方式
	Amount         int    `json:"amount,omitempty"`         // 支付金额, 单位分
	Currency       string `json:"currency,omitempty"`       // 货币代码
	State          int    `json:"state,omitempty"`          // 支付订单状态
	ClientIp       string `json:"clientIp,omitempty"`       // 客户端IPV4地址, 可选
	Subject        string `json:"subject,omitempty"`        // 商品标题
	Body           string `json:"body,omitempty"`           // 商品描述
	ChannelOrderNo string `json:"channelOrderNo,omitempty"` // 对应渠道的订单号, 可选
	ErrCode        string `json:"errCode,omitempty"`        // 渠道下单返回错误码, 可选
	ErrMsg         string `json:"errMsg,omitempty"`         // 渠道下单返回错误描述, 可选
	ExtParam       string `json:"extParam,omitempty"`       // 商户扩展参数, 可选
	CreatedAt      int64  `json:"createdAt,omitempty"`      // 订单创建时间, 13位时间戳
	SuccessTime    int64  `json:"successTime,omitempty"`    // 订单支付成功时间, 13位时间戳, 可选
	ReqTime        int64  `json:"reqTime,omitempty"`        // 通知请求时间, 13位时间戳
}
